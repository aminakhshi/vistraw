...
def kmc_ev(df, order, dt, lag, mode):
    # assert len(df.shape) == 2, ValueError('time series must have (n, dims) shape')
    # assert df.shape[0] > 0, ValueError('no data in time series')
    ...
    if mode == 'drift':
        ...
        ts1 = {}
        for i in range(dims):
            ts1['x{}'.format(i+1)] = np.array(df[:, i, :])

        for i in range(1, dims+1):
            # ts['y{0}'.format(i)] = ts1['x{0}'.format(i)][lag:] - ts1['x{0}'.format(i)][:(len(ts1['x{0}'.format(i)])-lag)]
            # ts['x{0}'.format(i)] = ts1['x{0}'.format(i)][:(len(ts1['x{0}'.format(i)])-lag)]
            ts['y{0}'.format(i)] = ts1['x{0}'.format(i)][-1,:] - ts1['x{0}'.format(i)][0,:]
            ts['x{0}'.format(i)] = ts1['x{0}'.format(i)][0,:]
...
##############################################################################################################################################
        ...
        ts1 = {}
        for i in range(1, dims+1):
            ts1['x{0}'.format(i)] = np.array(df[:, i-1, :])
